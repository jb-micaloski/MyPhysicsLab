...
2. Install the required Node tools:

        npm install

    This installs [TypeScript](https://www.typescriptlang.org) and
    [esbuild](https://esbuild.github.io) locally from `package.json`.

    + [Perl](https://www.perl.org)

    + [GNU Make](https://www.gnu.org/software/make/)

3. Execute `npx tsc` at the command line. This will compile all the typescript `.ts` files
...
Deploying on Vercel
-------------------
This repository includes a minimal Vercel configuration for publishing the
English build of the examples as a static site.
...
The build script runs TypeScript, generates the English HTML/JavaScript files
with `make LOCALE=en apps-en`, and copies `build/index-en.html` to
`build/index.html` so the deployment has a default root page.
